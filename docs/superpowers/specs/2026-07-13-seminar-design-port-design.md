...
- 코드 하이라이트(rouge) 배경은 **의도적 예외** — rouge 구문강조 팔레트가 어두운 배경용(밝은 텍스트)이라 `--subtle` 연동 시 라이트 모드에서 가독성이 깨짐(배포 후 Playwright 검증에서 확인). 코드블록은 라이트/다크 모두 테마의 어두운 배경을 유지하고 둥근 모서리만 적용
