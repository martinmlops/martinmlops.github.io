...
## 9. 다음 단계: 가드레일 전용 모델과의 비교

### 범용 SLM의 한계, 전용 모델의 가능성

이번 테스트에서 드러난 핵심 문제는, 믿음 Mini가 **범용 언어 모델**이라는 점입니다. 텍스트 생성, 대화, 요약 등 다양한 작업을 위해 학습된 모델에게 "입력 분류"라는 특수한 작업을 프롬프트만으로 시키는 건, 결국 모델의 설계 목적과 다른 용도로 쓰는 셈입니다.

반면, 최근에는 **가드레일/안전 분류를 위해 처음부터 설계된 전용 모델**들이 등장하고 있습니다. 이 모델들은 유해 콘텐츠 탐지, 프롬프트 인젝션 방어, 도메인 분류 등의 작업에 특화되어 있어, 범용 모델 대비 더 정확하고 빠른 분류가 가능합니다.

### 주요 가드레일 전용 모델

| 모델 | 개발사 | 파라미터 | 특징 |
|---|---|---|---|
| Llama Guard 4 | Meta | 다양한 규모 | 텍스트+이미지 입출력 모두 평가 가능. 커스텀 안전 정책 정의 지원 |
| Prompt Guard (86M) | Meta | 86M | 프롬프트 인젝션/탈옥 탐지 전용. 초경량으로 지연 시간 최소화 |
| ShieldGemma | Google | 2B/9B/27B | Gemma 기반 안전 분류기. 입출력 양방향 필터링 |
| Qwen3Guard | Alibaba | 1B/4B | 영어/중국어/다국어 안전 분류. Apache 2.0 라이선스 |

특히 주목할 점은 **Llama Guard 3-1B**의 벤치마크 결과입니다. arXiv:2601.19970에 따르면, 1B 파라미터의 경량 모델임에도 OWASP Top 10 LLM 위협 탐지에서 **76% 탐지율**을 기록했고, 추론 지연은 0.165초에 불과했습니다. 반면 범용 모델인 Llama-3.1-8B는 같은 테스트에서 0% 탐지율을 보였습니다. 파라미터 수가 8배 많아도, 가드레일 작업에 특화되지 않으면 무용지물이라는 뜻입니다.

```mermaid
graph LR
    subgraph "범용 SLM (이번 테스트)"
        A["믿음 Mini 2.3B<br>(범용 언어 모델)"] --> B["프롬프트로<br>분류 작업 지시"]
        B --> C["맥락 이해 부족<br>오분류 발생"]
    end
    subgraph "가드레일 전용 모델 (다음 테스트)"
        D["Llama Guard / ShieldGemma<br>(분류 전용 학습)"] --> E["안전 분류에<br>최적화된 추론"]
        E --> F["더 정확한 분류<br>더 빠른 추론"]
    end
```

### 다음 테스트 계획

후속 테스트에서는 동일한 군/국방 도메인 분류 작업에 대해 다음을 비교할 예정입니다:

| 비교 항목 | 믿음 Mini (범용) | 가드레일 전용 모델 |
|---|---|---|
| 분류 정확도 | 75% (12개 케이스) | 측정 예정 |
| 경계 케이스 처리 | 오분류 다수 | 측정 예정 |
| 추론 속도 | 수 초 | 측정 예정 |
| 커스텀 정책 적용 | 프롬프트 의존 | 정책 정의 API 지원 여부 |
| 한국어 지원 | 네이티브 | 다국어 지원 수준 확인 |

핵심 질문은 이것입니다: **가드레일 전용 모델이 한국어 도메인 분류에서도 범용 한국어 SLM보다 나은가?** 믿음 Mini는 한국어 이해력은 높지만 분류 판단력이 부족했습니다. 가드레일 전용 모델은 분류 판단력은 높지만 한국어 이해력이 부족할 수 있습니다. 이 트레이드오프를 실제 데이터로 검증하는 것이 다음 단계입니다.
...
- [Llama Guard 4 - HuggingFace Blog](https://huggingface.co/blog/llama-guard-4)
- [Llama Guard 벤치마크 (arXiv:2601.19970)](https://arxiv.org/html/2601.19970v1)
- [Qwen3Guard Technical Report](https://arxiv.org/html/2510.14276v1)
- [Meta Prompt Guard 86M - HuggingFace](https://huggingface.co/meta-llama/Prompt-Guard-86M)
