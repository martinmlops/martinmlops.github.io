...
toc: true
toc_sticky: true
excerpt: "'나 해고당했어'를 LinkedIn Speak으로 번역하면? '새로운 챕터로의 전환을 앞두고 있습니다.' Kagi Translate가 번역기의 정의를 다시 쓰고 있습니다. 248개 언어 지원, 프라이버시 퍼스트, 그리고 인터넷을 웃긴 스타일 트랜스퍼까지."
---
...
![Photo by Mika Baumeister on Unsplash](https://images.unsplash.com/photo-1611532736597-de2d4265fba3?w=1200&auto=format&fit=crop&q=80)
...
```mermaid
graph LR
    A["나 해고당했어"] --> K[Kagi Translate<br>LinkedIn Speak]
    K --> B["새로운 챕터로의<br>전환을 앞두고 있습니다"]
```
...
```mermaid
graph LR
    A["오늘 야근이야"] --> K[Kagi Translate]
    K --> B["INFP: 별이 뜰 때쯤<br>돌아갈게..."]
    K --> C["집착광인: 너 지금<br>어디야? 읽씹하지 마"]
    K --> D["사극: 소신이 오늘 밤<br>퇴궐이 어려울 듯하옵니다"]
```

```mermaid
graph TB
    subgraph "기존 번역기"
        T1[한국어] --> TR1[번역 엔진] --> T2[영어]
    end
    subgraph "Kagi Translate"
        S1[일반 텍스트] --> KT[AI 스타일 엔진]
        KT --> S2[LinkedIn Speak]
        KT --> S3[해적 말투]
        KT --> S4[셰익스피어]
        KT --> S5[특정 인물 문체]
        KT --> S6[248개 언어 번역]
...
```mermaid
graph LR
    subgraph "Google Translate"
        G1[텍스트 입력] --> G2[번역]
        G2 --> G3[결과]
        G1 --> G4[데이터 수집<br>광고 타겟팅]
    end
    subgraph "Kagi Translate"
        K1[텍스트 입력] --> K2[번역]
        K2 --> K3[결과]
        K1 --> K4["데이터 수집 없음<br>추적 없음"]
    end
```
...
![Photo by Towfiqu barbhuiya on Unsplash](https://images.unsplash.com/photo-1614064641938-3bbee52942c7?w=1200&auto=format&fit=crop&q=80)
...
```mermaid
graph TD
    P[과거: 번역 = 언어 A → 언어 B] --> N[현재: 번역 = 의미 + 스타일 + 톤 + 맥락]
    N --> F["미래: 번역 = 범용 커뮤니케이션<br>스타일 엔진"]
```
...
![Photo by Volodymyr Hryshchenko on Unsplash](https://images.unsplash.com/photo-1543269865-cbf427effbad?w=1200&auto=format&fit=crop&q=80)
