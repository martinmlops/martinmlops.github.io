...
글로벌 Layer 7 로드 밸런서로, Microsoft의 전 세계 100개 이상 PoP(Point of Presence, 접속 거점)를 활용합니다. PoP는 전 세계 주요 도시에 분산 배치된 Microsoft의 엣지 네트워크 노드로, 사용자와 가장 가까운 위치에서 요청을 수락하여 지연시간을 최소화합니다.
...
- **세션 어피니티**: 동일 사용자의 요청을 동일 백엔드로 라우팅

### 트래픽 라우팅 방법

| 라우팅 방법 | 설명 | 사용 사례 |
|-------------|------|-----------|
| Priority | 우선순위 기반 | 재해 복구 (DR) |
| Weighted | 가중치 기반 분산 | A/B 테스팅, 카나리 배포 |
| Performance | 지연시간 최소화 | 글로벌 서비스 |
| Geographic | 지리적 위치 기반 | 데이터 주권 (GDPR 준수) |

### Front Door vs CDN

| 기능 | Front Door | Azure CDN |
|------|-----------|-----------|
| 동적 콘텐츠 가속 | ✅ | ❌ |
| WAF 통합 | ✅ (네이티브) | ✅ (제한적) |
| 글로벌 로드 밸런싱 | ✅ | ❌ |
