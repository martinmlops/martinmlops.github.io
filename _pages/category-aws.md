...
AWS 클라우드 서비스와 AI 기반 개발 방법론에 대한 학습 자료입니다.

## 하위 카테고리

{% assign cat_posts = site.posts | where_exp: "post", "post.categories contains 'AWS'" %}
...
- [{{ sub }}](/categories/aws/{{ sub_slug }}/) <small>({{ count }})</small>
{% endfor %}

---

{% assign all_posts = site.posts | where_exp: "post", "post.categories contains 'AWS'" | sort: "date" | reverse %}
